...
module.exports = {
    init(client) {
        ...
        client.on('interactionCreate', async (interaction) => {
            ...
            if (interaction.customId.startsWith('event_join_')) {
                try {
                    // 1. ZABEZPIECZENIE TECHNICZNE (Zapobiega Unknown Interaction)
                    if (interaction.replied || interaction.deferred) return;
                    await interaction.deferUpdate();

                    // 2. PRZYCISK "ZAJĘTE" (Szary, Disabled - tak jak chciałeś)
                    ...
                    // Używamy editReply bo zrobiliśmy deferUpdate
                    ...
                    // Ignorujemy błędy 10062 (Unknown interaction), bo są niegroźne przy deferze
                    if (e.code !== 10062) console.error("❌ Błąd przycisku eventu:", e);
                }
            }
        });

        // Pętla czasowa (bez zmian)
        setInterval(async () => {
    ...
    async triggerEvent(client) {
        try {
            ...
            // PRZYWRÓCONO STARY WYGLĄD WIADOMOŚCI
    ...
    async createPrivateQuestion(interaction, kategoria) {
        ...
        try {
            ...
            // PRZYWRÓCONO STARE TWORZENIE NAZWY KANAŁU (z nickiem usera)
            ...
            // FollowUp (bo defer był wcześniej)
            ...
            // Proste sortowanie (jak w oryginale)
            ...
            collector.on('collect', async (i) => {
                ...
                // ZABEZPIECZENIE PRZED UNKNOWN INTERACTION W KANALE
                if (!i.deferred && !i.replied) await i.deferUpdate();

                if (i.customId === 'q_correct') {
                    // Używamy editReply zamiast update (bo deferUpdate było wyżej)
                    await i.editReply({ 
