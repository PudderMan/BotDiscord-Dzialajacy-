...
module.exports = {
    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() === 'config') {
            ...
            return interaction.reply({ content: `✅ Zaktualizowano ${typ} o ${val}.`, ephemeral: true });
        }
    ...
    async handleInteraction(interaction) {
        ...
        if (interaction.customId === 'click_proch') {
            ...
            const freshProch = db.prepare('SELECT proch FROM players WHERE userId = ?').get(userId).proch;

            const newEmbed = EmbedBuilder.from(interaction.message.embeds[0]).setFields(
                { name: '✨ Proch:', value: `${formatNum(freshProch)}g`, inline: true },
        ...
        if (interaction.customId === 'open_shop' || interaction.customId.startsWith('shop_p')) {
            ...
            return interaction.customId === 'open_shop' 
                ? await interaction.reply({ embeds: [sEmbed], components: rows, ephemeral: true }) 
                : await interaction.update({ embeds: [sEmbed], components: rows });
        }
        ...
        if (interaction.customId === 'buy_paczka') {
            ...
            return interaction.reply({ content: "🚀 PACZKA ODPALONA! Mega Boost x10 aktywny!", ephemeral: false });
        }
        ...
        if (interaction.customId === 'firework_boom' && data.proch >= nextPresPrice) {
            ...
            return interaction.reply({ content: "🎆 Wystrzelono! Zyskujesz 1 🎇", ephemeral: true });
        }
